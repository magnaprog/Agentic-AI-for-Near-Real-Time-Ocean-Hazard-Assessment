#!/usr/bin/env python3
"""Retrospective validation: run the anomaly detection pipeline on Iquique 2014 DART data.

Reads CSV files produced by download_iquique_dart.py (event + calibration windows),
...
Usage:
    python scripts/validate_iquique.py [--data-dir data/iquique]

Prerequisites:
    1. pip install -e .
    2. python scripts/download_iquique_dart.py --calibration-days 30
"""
...
_EQ = load_seismic_params("iquique")
IQUIQUE_ORIGIN_UTC = _EQ.origin_utc
IQUIQUE_MAGNITUDE = _EQ.magnitude
IQUIQUE_LAT = _EQ.latitude
IQUIQUE_LON = _EQ.longitude
IQUIQUE_DEPTH_KM = _EQ.depth_km
...
def validate_station(
    ...
    """Run anomaly detection on a single station's Iquique data."""
    ...
    agent.update_seismic_events([
        SeismicEvent(
            event_id="usc000nzvd",
            magnitude=IQUIQUE_MAGNITUDE,
            origin_time=IQUIQUE_ORIGIN_UTC,
            latitude=IQUIQUE_LAT,
            longitude=IQUIQUE_LON,
        ),
    ])
    ...
    scores, spatial_result = agent.process_station_data(
        ...
        processing_time=IQUIQUE_ORIGIN_UTC,
    )
...
def sliding_window_analysis(
    ...
    pre_event_minutes = (IQUIQUE_ORIGIN_UTC.timestamp() - event_t0) / 60.0
    ...
    for end_idx in range(samples_per_step, len(event_values), samples_per_step):
        ...
        fsm_monitoring = cutoff_epoch >= IQUIQUE_ORIGIN_UTC.timestamp()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate anomaly detection on Iquique 2014 DART data",
    )
    parser.add_argument(
        "--data-dir", type=str, default="data/iquique",
        help="Directory with station CSV files from download_iquique_dart.py",
    )
    parser.add_argument(
    ...
    if not data_dir.exists():
        logger.error(
            "Data directory %s not found. Run download_iquique_dart.py first.", data_dir,
        )
        return

    event_files = sorted(data_dir.glob("dart_*_iquique_2014_event.csv"))
    ...
    for event_path in event_files:
        ...
        cal_path = data_dir / f"dart_{station_id}_iquique_2014_calibration.csv"
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("Iquique 2014 Retrospective Validation Summary")
    print(f"{'='*100}")
    print(
    ...
    if n_investigate > 0:
        print("\nRESULT: System would have detected the Iquique tsunami signal.")
    else:
    ...
    output = {
        "event": "Iquique 2014",
        "origin_utc": IQUIQUE_ORIGIN_UTC.isoformat(),
        "magnitude": IQUIQUE_MAGNITUDE,
        "depth_km": IQUIQUE_DEPTH_KM,
        "epicenter": {"lat": IQUIQUE_LAT, "lon": IQUIQUE_LON},
        "per_station": results,
        "summary": {
    ...
    output_path = results_dir / "iquique_detection.json"
