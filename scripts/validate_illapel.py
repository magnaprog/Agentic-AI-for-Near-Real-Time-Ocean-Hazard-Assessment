#!/usr/bin/env python3
"""Retrospective validation: run the anomaly detection pipeline on Illapel 2015 DART data.

Reads CSV files produced by download_illapel_dart.py (event + calibration windows),
...
Usage:
    python scripts/validate_illapel.py [--data-dir data/illapel]

Prerequisites:
    1. pip install -e .
    2. python scripts/download_illapel_dart.py --calibration-days 30
"""
...
_EQ = load_seismic_params("illapel")
ILLAPEL_ORIGIN_UTC = _EQ.origin_utc
ILLAPEL_MAGNITUDE = _EQ.magnitude
ILLAPEL_LAT = _EQ.latitude
ILLAPEL_LON = _EQ.longitude
ILLAPEL_DEPTH_KM = _EQ.depth_km
...
def validate_station(
    ...
    """Run anomaly detection on a single station's Illapel data."""
    ...
    agent.update_seismic_events([
        SeismicEvent(
            event_id="us20003k7a",
            magnitude=ILLAPEL_MAGNITUDE,
            origin_time=ILLAPEL_ORIGIN_UTC,
            latitude=ILLAPEL_LAT,
            longitude=ILLAPEL_LON,
        ),
    ])
    ...
    scores, spatial_result = agent.process_station_data(
        ...
        processing_time=ILLAPEL_ORIGIN_UTC,
    )
...
def sliding_window_analysis(
    ...
    pre_event_minutes = (ILLAPEL_ORIGIN_UTC.timestamp() - event_t0) / 60.0
    ...
    for end_idx in range(samples_per_step, len(event_values), samples_per_step):
        ...
        fsm_monitoring = cutoff_epoch >= ILLAPEL_ORIGIN_UTC.timestamp()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate anomaly detection on Illapel 2015 DART data",
    )
    parser.add_argument(
        "--data-dir", type=str, default="data/illapel",
        help="Directory with station CSV files from download_illapel_dart.py",
    )
    parser.add_argument(
    ...
    if not data_dir.exists():
        logger.error(
            "Data directory %s not found. Run download_illapel_dart.py first.", data_dir,
        )
        return

    event_files = sorted(data_dir.glob("dart_*_illapel_2015_event.csv"))
    ...
    for event_path in event_files:
        ...
        cal_path = data_dir / f"dart_{station_id}_illapel_2015_calibration.csv"
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("Illapel 2015 Retrospective Validation Summary")
    print(f"{'='*100}")
    print(
    ...
    if n_investigate > 0:
        print("\nRESULT: System would have detected the Illapel tsunami signal.")
    else:
    ...
    output = {
        "event": "Illapel 2015",
        "origin_utc": ILLAPEL_ORIGIN_UTC.isoformat(),
        "magnitude": ILLAPEL_MAGNITUDE,
        "depth_km": ILLAPEL_DEPTH_KM,
        "epicenter": {"lat": ILLAPEL_LAT, "lon": ILLAPEL_LON},
        "per_station": results,
        "summary": {
    ...
    output_path = results_dir / "illapel_detection.json"
