#!/usr/bin/env python3
"""Retrospective validation: run the anomaly detection pipeline on Chile 2010 DART data.

Reads CSV files produced by download_chile_dart.py (event + calibration windows),
...
Prerequisites:
    1. pip install -e .
    2. python scripts/download_chile_dart.py --calibration-days 30
"""
...
def validate_station(
    ...
    """Run anomaly detection on a single station's Chile data.
    ...
    # Provide seismic context (Chile earthquake)
    agent.update_seismic_events([
        SeismicEvent(
            event_id="us2010chile",
            magnitude=CHILE_MAGNITUDE,
            origin_time=CHILE_ORIGIN_UTC,
            latitude=CHILE_LAT,
            longitude=CHILE_LON,
        ),
    ])
    ...
    scores, spatial_result = agent.process_station_data(
        ...
        processing_time=CHILE_ORIGIN_UTC,
    )
...
def sliding_window_analysis(
    ...
    pre_event_minutes = (CHILE_ORIGIN_UTC.timestamp() - event_t0) / 60.0
    ...
    for end_idx in range(samples_per_step, len(event_values), samples_per_step):
        ...
        fsm_monitoring = cutoff_epoch >= CHILE_ORIGIN_UTC.timestamp()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate anomaly detection on Chile 2010 DART data",
    )
    parser.add_argument(
        "--data-dir", type=str, default="data/chile",
        help="Directory with station CSV files from download_chile_dart.py",
    )
    parser.add_argument(
    ...
    if not data_dir.exists():
        logger.error(
            "Data directory %s not found. Run download_chile_dart.py first.", data_dir,
        )
        return

    # Find event files
    event_files = sorted(data_dir.glob("dart_*_chile_2010_event.csv"))
    ...
    for event_path in event_files:
        ...
        cal_path = data_dir / f"dart_{station_id}_chile_2010_calibration.csv"
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("Chile 2010 Retrospective Validation Summary")
    print(f"{'='*100}")
    print(
    ...
    if n_investigate > 0:
        print("\nRESULT: System would have detected the Chile tsunami signal.")
    else:
    ...
    output = {
        "event": "Chile 2010",
        "origin_utc": CHILE_ORIGIN_UTC.isoformat(),
        "magnitude": CHILE_MAGNITUDE,
        "depth_km": CHILE_DEPTH_KM,
        "epicenter": {"lat": CHILE_LAT, "lon": CHILE_LON},
        "per_station": results,
        "summary": {
    ...
    output_path = results_dir / "chile_detection.json"
