...
report stages sit downstream in the specification, and no runtime path
assembles their inputs.  Report generation is gated on nine
verification checks, and nothing outside the offline demo scripts
assembles a verification input, so no report is produced on a live or
archived stream.
...
Two processing agents, QC and Anomaly, run in sequence under
deterministic FSM control.  Scenario, Verification and Report are
specified with the same typed handoff schemas, and no runtime path
invokes them (Figure~\ref{fig:pipeline}).
...
come after stages no runtime path assembles input for.
...
no run of them is profiled.  Measurements come from
...
in ${\sim}0.12$\,ms, and FSM transitions are sub-millisecond. Against a 1-minute event-mode sampling interval, single-station
...
\caption{Component diagram showing the internal structure of the
  ...
  no runtime path assembles their inputs today. The deterministic FSM
