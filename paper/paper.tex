...
though only the DART path is exercised end to end here.  A
deterministic finite-state machine orchestrates quality control,
...
A scenario that fails verification routes to ABSTAIN, which suppresses
probabilistic guidance and sends a structured abstention to the
mandatory Human Review Gate instead.
An optional LLM layer synthesizes operator-readable narratives but
cannot modify detection state or bypass verification.  While an event is open, an
...
anomaly detection, scenario ranking, and structured communication, and
produce human-interpretable probabilistic guidance. The challenge is
...
classified as approved without an explicit review decision.
...
\begin{enumerate}
  ...
  \item Automated Rayleigh wave false-trigger
    ...
    them.
  \item A three-plane architecture (Data, Processing, Output)
...
Four processing agents run in sequence under deterministic FSM control
and hand off to one another through typed schemas; a fifth agent
(Report) runs only when verification routes to it
...
when verification fails. Reports and abstentions alike are presented at
...
work. Every audited system event is appended to an append-only audit log,
held in memory and persisted to TimescaleDB when a database is
...
transitions on live streams.  The flow below is the target: its
ingest, QC, anomaly and FSM stages have been exercised on archived
data (Section~\ref{sec:evaluation}), and its Scenario, Verification
and Report stages remain future work that no path runs on live or
archived streams.
The seismic connector detects a qualifying earthquake within 15\,s of
USGS publication, and the FSM transitions to MONITOR. The DART connector polls standard-mode feeds
...
effect at the time. The FSM is not thread-safe internally; the FastAPI
...
constraints). The active stage is recorded with the assessment and
...
seismic-only estimate is computed from magnitude scaling
...
PROVISIONAL and non-distributable throughout: a review decision is
...
and pipeline outcomes.  Each entry is an \texttt{AuditEntry} carrying a UUID,
...
permissions). Each connector records a SHA-256 hash of the raw payload at the ingest
...
metadata; the Tohoku validator does not run it, and the event-mode
status of all eight stations comes from the archive's
measurement-type codes.  The Scenario Agent would then produce a seismic-only
...
Table~\ref{tab:latency} reports component latencies.  Ingest, QC,
detiding, anomaly scoring and the FSM transition are measured;
scenario inversion and both report paths are design targets, because
no run of them is profiled here.  Measurements come from
...
$\sim$47\,ms at p95, which subtraction attributes almost entirely to
the BOCPD computation ($\sim$44\,ms). QC checks process 10 records
...
south (150--189\textdegree), near that radiation maximum, so their
${\sim}9\times$ larger amplitudes (0.23--0.28\,m) come from Green's Law
shoaling (${\sim}3.8$--$4.5\times$ at nearshore depths of 10--20\,m)
multiplied by a directivity factor about $2.4\times$ higher than the
...
the compared methods do not. No runtime path in this release
...
the 3-minute figure is a design target for a pre-loaded database, not
a measurement, and this release fixes no bound on the candidate
subfault count. When the LLM synthesis layer is enabled a single API
...
point is that no runtime path assembles a \texttt{VerificationInput}
at all: the pipeline node that would run these checks reads a
...
reviewer packet of record, and fail-closes to ABSTAIN at the
assessment stages.
...
and de-escalating on threshold rules plus a seismic override; an ensemble anomaly
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Center}: Ocean Map (Leaflet-based Pacific view with
    ...
    score the console receives, with $T_1$/$T_2$/$T_3$
...
51407 lies inside the six-hour tsunami reach, and only just, with a
predicted arrival in the last minutes of the window, so its zero score
does not separate the two causes either.
