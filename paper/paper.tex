...
\begin{enumerate}
  ...
  \item An evidence investigator that acts while an event is open,
    ...
    database role, and the role that drives the state machine cannot
    write them, so a finding cannot re-enter the decision path.
...
deterministic report intact.  The test suite exercises 67~guardrail test functions with
118~assertions covering prohibited-term detection, disclaimer
...
dedicated \texttt{investigator\_writer} role alone; the \texttt{pipeline\_worker}
role that drives the FSM has no insert privilege on it, and neither does the
API's own role, so a finding cannot be read back as evidence by the code that
escalates.  Each is verified against a provisioned database rather than
assumed.  The investigator also reads a checkpoint the worker has already
persisted, so it sits off the detection path and cannot add latency to
ingest, scoring, or a transition, and every finding passes the same guardrail
