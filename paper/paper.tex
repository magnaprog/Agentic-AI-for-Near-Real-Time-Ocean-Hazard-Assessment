...
TDA, and Fast Iterative Filtering), which address seismic
contamination primarily through spectral or temporal separation rather
than through source-distance timing.
...
transitions on live streams; full live assessment operation (Scenario,
Verification, Report) remains future work, and the flow below has been
exercised only on archived data (Section~\ref{sec:evaluation}).
...
follow large earthquakes.  A clean check passes, and one whose
prerequisites are missing records as not evaluated.
...
granted UPDATE or DELETE on the audit tables, the four baseline roles
have both explicitly revoked, and a database trigger blocks them
...
alone: the FSM transitions IDLE~$\to$~MONITOR~$\to$~ESCALATE on the
USGS trigger, placing the event in human review before any DART data
...
independent tsunami confirmation.  The QC Agent
...
Two audit entries follow from the FSM rules and the seismic
parameters alone: \texttt{state\_transition} IDLE~$\to$~MONITOR on the
$M$\,9.1 trigger, then \texttt{state\_transition}
MONITOR~$\to$~ESCALATE on the seismic-only path (depth 29\,km
$< 100$\,km).  The verification and report entries that the full flow
would add come after stages no runtime path assembles input for, so
this event has none.
...
\caption{Detided residuals after removing the 8-constituent harmonic
  ...
  Processing applied to the residual: despiking, pre-event linear
  detrend, a 120\,min centered rolling median for stations beyond
  3{,}000\,km to remove residual tidal curvature, and a Hampel spike
  filter.  This figure shows the detided residual
...
Table~\ref{tab:latency} reports component latencies, measured where the component is local and stated as design targets where it is network-bound, from
...
fault strike (250\textdegree) directs maximum radiation NNW while the
Pacific stations lie south or east of the epicenter and receive
...
interval (15--60\,s). None of the methods in Table~\ref{tab:method-comparison} integrates
...
time, and falls back to the template-only path in $<1$\,s when the API is
...
compared with a 236.4~min reference at 2{,}803~km. Event-mode status is
...
set \texttt{abstain\_required} and route to ABSTAIN.  At this stage, though, the gate has very little evidence to act on.
...
Gate blocks autonomous public communication. The permission matrix documents each component's capability envelope
...
and de-escalating on threshold rules alone; an ensemble anomaly
...
2009) shows the ensemble detector crossing the fixed operating-point
thresholds at the nearest event-mode DART station in each event,
across subduction-zone thrust and outer-rise normal-faulting
mechanisms.
...
\caption{Component diagram showing the internal structure of the
  ...
  Scenario and Verification agents, which exist as components that no
  runtime path currently feeds. The deterministic FSM sits beside them
  as the sole state authority. The Output Plane includes the Report Agent,
...
escalation packet of record: the checkpoint, the transition, the best
scored station, and the assessment's identifier and content hash.  The
assessment body is persisted but not served to the console.
...
clamped passband degrades detection, and all but one of them sit
beyond the distance a tsunami covers in the six-hour window, so a low
score here measures neither effect on its own.
...
Standard-mode stations produce near-zero scores.  The pipeline's
\texttt{filter\_degraded} flag tracks the clamped passband
(Table~\ref{tab:chile-results}), but these stations also lie beyond
the six-hour tsunami reach, so the two causes cannot be separated
here.
...
The three far-field standard-mode stations are filter degraded, and
one of them, 51407, is close enough that the clamped passband is a
live explanation for its low score.
