...
path executes.
Verification failure is specified to route to ABSTAIN.  The case that
fires today is the missing one: the worker reaches ASSESS or ESCALATE
with no scenario to verify, abstains, and records the abstention for
review.
...
tsunami arrival to fall inside the six-hour evaluation window, so the low
...
can be audited after the fact, and a review decision recorded against
every escalation.
...
verification checks, one of which has a prerequisite nothing
satisfies, so no report is produced on a live or archived stream.
...
\begin{enumerate}
  ...
  \item A fail-closed verification specification of nine checks
    ...
    missing verification; the checks themselves have not run outside
    unit tests.
  \item A publicly available implementation, free for noncommercial use
    ...
    communication, a recorded review decision against every escalation
    packet, deterministic detection).
\end{enumerate}
...
(Section~\ref{sec:evaluation}); the replay scripts compare scores
against the same threshold constants the FSM uses rather than running
ingest, QC or the orchestrator, and the Scenario, Verification and
...
observation as it enters the detection pipeline.  Not all five
evaluate on every record: gross range returns NOT\_EVALUATED for raw
water-column heights, and the flat-line test needs a full rolling
window. The resulting flags
...
(Table~\ref{tab:fsm}).  Two paths do not use the score: a seismic
override ($M \geq 7.5$ with DART event-mode transmission) advances the
FSM one state per evaluation and takes priority over de-escalation,
and a seismic-only chain ($M \geq 7.5$, depth $< 100$\,km) escalates
...
No role holds UPDATE or DELETE on \texttt{audit\_events}, the four
baseline roles have both explicitly revoked, and a row-level trigger
raises on either regardless of grants. Each connector records a SHA-256 hash of the raw payload at the ingest
...
$\sim$47\,ms at p95.  Detection over an already detided residual takes
44\,ms of that, against 0.06\,ms for the separately profiled wavelet
row, so BOCPD accounts for nearly all of the detector's cost. QC checks process 10 records
...
the 3-minute figure is a design target for a pre-loaded database
rather than a measurement, over the at most 15 candidate unit sources
the selector returns by default. When the LLM synthesis layer is enabled a single API
...
51407 is close enough for the question to arise, and its predicted
arrival at 357.5\,min still falls just past the end of its 356.8\,min
record, so its zero score does not separate the two causes either.
