...
TDA, and Fast Iterative Filtering), all of which separate seismic
contamination spectrally or temporally rather than by source-distance
timing.
...
work. Every audited system event is appended to that log, held in memory
...
summarized in Algorithm~\ref{alg:residual}.  Two further steps act on
the event residual, where the algorithm shows their equivalents only
for the calibration window: an optional rolling-median baseline
correction whose window depends on epicentral distance and event, and
a Hampel spike clean.
...
missing, malformed, and FAIL verification results all route to the
ABSTAIN path.  The missing case is the one that fires today, since the
...
write to an operational feed; it has no dissemination or physical
actuation capability of any kind. No role is granted UPDATE or DELETE
on the audit tables, the four roles that predate the assessment tables
...
\label{sec:audit}
Each FSM transition, review decision, abstention, and pipeline outcome
is appended to an audit log as an \texttt{AuditEntry} carrying a UUID,
...
boundary and carries it in the handoff schemas, so a downstream record
can be matched back to the observation it came from. The hash is stored
alongside the record it covers, so it detects a value changed in
processing rather than an actor with write access. FSM transition and decision audit metadata is queryable by event ID or trace
...
Event mode alone does not produce a crossing: 46403 for
T\={o}hoku and 46412 for Chile stay below $T_1$, so distance and
...
Verification Agent evaluates the nine-check matrix, though at the
seismic-only constraint stage most of those checks either do not apply
or have no input.  The Report Agent renders the deterministic template
report, and the LLM advisory layer, when enabled, attaches a narrative
...
\caption{Detided residuals after removing the 8-constituent harmonic
  ...
  pressure perturbations; 46408 sits at the boundary, with its
  predicted arrival near the end of the record.
  Processing: level-shift correction,
...
ASSESS & INVESTIGATE & $s < T_2$ (de-escalation; suppressed when $M \geq 7.5$ with DART event-mode$\to$ESCALATE) \\
\addlinespace
