...
capability envelope in a machine-readable permission matrix, and
enforces the safety properties themselves through restricted database
write access, structural separation of the LLM layer from the control
path, and a mandatory Human Review Gate. The ABSTAIN path
...
producing a false event-mode trigger. This is an amplitude/timing
...
%  them visually above centre and well clear of the de-escalation arrows below.
\draw[fwd]
...
Sensitivity (LOO)    & Active-set flips on           & Weight shift   \\
                     & leave-one-out                  & $>30\%$        \\
Posterior stability  & ---                           & Source set     \\
                     &                               & changed        \\
...
Model fit            & RMSE $> 5$\,cm or bias        & RMSE $> 3$\,cm \\
                     & $> 1$\,cm AND $> 3\sigma$     &                \\
Tidal state          & Needed + uncorrected           & Uncorrected    \\
                     &                               & coastal proxies\\
Meteotsunami         & Non-tsun.\ energy $> 60\%$    & $> 30\%$       \\
...
terms reserved for official NOAA products~\citep{nwspd107}:
``Warning,'' ``Advisory,'' ``Watch,'' ``Information Statement,''
``Threat Message'' (an international TWC product type),
``Cancellation,'' ``All Clear,'' and ``Bulletin.'' Prior to matching,
...
checks still execute, but none of them can currently fail: no component
supplies a meteotsunami score or sets the tidal-correction flag, so
those two checks always report NOT\_EVALUATED with a MISSING
prerequisite, and Rayleigh screening returns CONCERN by construction.
A side effect is that PASS is unreachable in the shipped
configuration, because an OPTIONAL check without its prerequisite
degrades the aggregate to PASS\_WITH\_CONCERNS; every computed outcome
we record is PASS\_WITH\_CONCERNS for this reason.
...
four operated in event mode (1-min or 45-sec sampling) during the
event window.
