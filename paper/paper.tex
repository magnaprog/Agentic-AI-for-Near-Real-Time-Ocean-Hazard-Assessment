...
NOAA's operational warning systems provide the authoritative
forecasts.  This work targets the step upstream of them: the manual
integration of heterogeneous feeds that precedes a duty scientist's
judgment.  We automate that integration rather than replace an
...
Connectors for NOAA DART bottom-pressure records, CO-OPS water-level
observations, and USGS seismic event feeds all ship with the system,
though only the DART path is exercised on archived data here.  A
deterministic finite-state machine orchestrates quality control and
ensemble anomaly detection.  NNLS-based scenario
inversion~\citep{lawson1974solving, percival2011extraction} and a
nine-check verification stage ship as specified stages that no runtime
path executes.
Verification failure is specified to route to ABSTAIN.  What fires
today is the missing-verification case: the worker reaches ASSESS or
ESCALATE with no scenario to verify, abstains, and records the
abstention for review.
An optional LLM layer synthesizes operator-readable narratives but
cannot modify detection state or bypass verification.  On request while
an event is open, and only once the worker has persisted a checkpoint,
an investigator chooses its own read-only queries over the recorded
...
tsunami arrival to fall inside the six-hour evaluation window.  For
that one the predicted arrival lands after its last sample, so the low
scores cannot be attributed to sampling rate alone.  All five events
were available during development, so this is a retrospective
consistency check rather than a held-out test.
...
T\={o}hoku event caused 19,759 confirmed deaths with 2,553 persons still missing~\citep{npa2023tohoku}. Field surveys recorded
...
gica2008development}. These systems meet the validation standards required for life-safety
products and are the result of decades of operational development.
...
and decision-makers. Under the operational pressure of a developing event, this manual
integration is slow and error-prone. In the fastest cases duty
scientists may have as few as 3--5 minutes between initial seismic
detection and the need for a preliminary
assessment~\citep{bernard2015evolution}, with dozens of sensor streams
spread across multiple visualization tools.
...
in safety-critical hazard assessment. An agentic framework can constrain LLM involvement to advisory roles
and use deterministic signal processing for detection and assessment.
Such a framework can automate the integration step together with
anomaly detection, and produce human-interpretable guidance. The challenge is
...
can be audited after the fact, and any review decision bound to the
escalation packet it was made against.
...
data, and USGS seismic event catalogs. The retrospective evaluation in
this paper uses DART bottom-pressure records; the CO-OPS and USGS
connectors are implemented and unit-tested but are not yet exercised in
the deployed worker or evaluated on historical events. A
five-state deterministic finite-state machine (FSM) orchestrates a
sequential pipeline whose deployed portion is quality control and
ensemble anomaly detection; the scenario-inversion, verification and
report stages sit downstream in the specification.  Report generation
is gated on nine verification checks, and nothing outside the offline
demo scripts assembles an input for any of those three stages, so no
report is produced on a live or archived stream.
No assessment is ever classified as approved: the review endpoint
records a caller-asserted decision against the escalation packet and
changes nothing else.
A bounded LLM advisory layer provides optional narrative synthesis
through a four-node graph built with LangGraph~\citep{langgraph2024}
and a tool-use-driven after-action analysis, both restricted to
read-only operations. Deterministic templates remain the report of
record, and LLM output is stored as separate advisory commentary that
is dropped on any failure or guardrail violation.

\textbf{Contributions.} This work makes two primary contributions
(items 1--2), four domain applications of established patterns
(items 3--6), and a public release (item 7). Together they show how
established safety-engineering and AI techniques can be combined in a
bounded agentic architecture for tsunami assessment:
...
\begin{enumerate}
  ...
  \item Automated Rayleigh wave false-trigger
    ...
    processing and can run alongside those filters rather than replace
    them.  It runs in the deployed worker, but the retrospective
    evaluation supplies no station coordinates, so no result reported
    here carries a Rayleigh verdict.
  \item A three-plane architecture (Data, Processing, Output) with a
    machine-readable but non-enforcing agent permission declaration and
    append-only audit and assessment tables.  Raw-observation
    provenance is grant-protected rather than trigger-protected and is
    not yet carried end to end. Related architectural patterns include the
  ...
  \item An ensemble anomaly detector with three core components
    ...
    coherence as an input feature.  No shipped path loads a model, so
    the ML component affects no number reported here.
  \item A fail-closed verification specification of nine checks
    (including Rayleigh wave suspect detection) with an explicit
    ABSTAIN output.  The routing defense is live and abstains today on
    missing verification; the checks themselves run only in unit tests
    and in the offline demo script.
  \item A publicly available implementation, free for noncommercial use
    ...
    communication, review decisions bound to the packet of record,
    deterministic detection).
\end{enumerate}

\noindent We evaluate the system through retrospective replay on five
...
\citet{percival2011extraction} developed a
...
from offshore observations using neural networks, and report comparable
...
seconds. These ML-based systems point toward faster tsunami forecasting. Our
system is complementary: it orchestrates DART observations within an
agentic pipeline whose scenario stage applies the unit-source
Green's-function inversion method of SIFT~\citep{titov2005realtime},
and it keeps that path deterministic and auditable.

\paragraph{Oceanographic data quality.}
The U.S. Integrated Ocean Observing System (IOOS) maintains the Quality
Assurance of Real-Time Oceanographic Data (QARTOD) standard~\citep{ioos2021qartod}, which defines a set of quality
...
streaming data without requiring a fixed window. That property suits
tsunami onset, where the shift appears before the full waveform
arrives. Isolation
...
TDA, and Fast Iterative Filtering).
...
Rayleigh wave detector exploits. Earlier work by the first author
...
primarily through spectral separation. Our Rayleigh wave detector works on timing instead: rather than
filtering Rayleigh energy out of the signal, it predicts the
surface-wave arrival time from the epicentral distance and flags any
trigger that falls inside that window.

\paragraph{Dual-plane and safety architectures.}
Isolating a high-assurance safety core from an AI-driven primary
function is an established pattern in safety engineering.
...
to LLM advisory agents in the tsunami warning domain. LLM agents
introduce distinct failure modes (hallucination, prompt injection,
confidence miscalibration) that call for enforcement mechanisms beyond
those designed for ML perception systems.
...
implementation evaluation. Our deterministic safety core is a structured workflow
(FSM-orchestrated, no LLM-directed control flow).  LLM-directed tool
use is confined to two components, the active-event evidence
investigator and the after-action analysis graph, where the model
selects which audit queries to run from the current analysis
context. We apply the principle of \emph{bounded
...
This constraint is enforced by architectural separation,
...
prediction data. It produced 150~draft forecasts per day, each
post-edited by a meteorologist, a division of labor analogous to our
Human Review Gate.  With modern LLMs, \citet{lawson2025pixels}
evaluated GPT-4V for generating severe-weather outlooks from
meteorological imagery and documented hallucination and reasoning
errors of the kind that motivate safety guardrails.
...
% Kafka - event bus carrying observations up into the Processing Plane.
% This is the live path: the pipeline worker consumes the topic. The
% database is a side branch, so Kafka sits directly under the agents and
% TimescaleDB is off to the right.
\node[store, fill=red!6, minimum width=8cm] (kafka) at (-1.2, -0.55) {Kafka};

% TimescaleDB - durable store written by the connectors and by the worker.
% Nothing reads raw observations back out of it.
\node[store, minimum width=2.6cm] (tsdb) at (5.5, -0.55) {TimescaleDB};

% Ingest connectors - evenly spaced under Kafka
\node[box] (dart)  at (-3.9, -1.55) {DART\\Ingest};
\node[box] (coops) at (-1.2, -1.55) {CO-OPS\\Ingest};
\node[box] (usgs)  at ( 1.5, -1.55) {Seismic\\Ingest};

% Ingest -> Kafka, carrying the raw.observations topic
\draw[arrow] (dart.north)  -- ([xshift=-2.7cm]kafka.south);
\draw[arrow] (coops.north) -- (kafka.south)
  node[font=\tiny, midway, right, xshift=2pt] {\texttt{raw.observations}};
\draw[arrow] (usgs.north)  -- ([xshift= 2.7cm]kafka.south);

% Ingest -> TimescaleDB (each connector also persists the raw record)
\draw[arrow] (usgs.east) -| (tsdb.south)
  node[font=\tiny, pos=0.25, above] {raw observations};
...
% Kafka -> QC (the worker consumes the topic; this is the live path)
\draw[arrow] ([xshift=-3.3cm]kafka.north) -- (qc.south);

% Processing plane -> TimescaleDB (features and audit rows written back)
\draw[arrow] ([xshift=6.2cm]proc.south) -- ([xshift=0.7cm]tsdb.north)
  node[font=\tiny, midway, left, xshift=-2pt] {features + audit};
...
\end{tikzpicture}
\caption{System architecture and agentic pipeline. In the
  \textbf{Data Plane}, each connector publishes to the Kafka
  \texttt{raw.observations} topic and separately writes the raw record to
  TimescaleDB; one such write is drawn, standing for all three.  The worker consumes the topic, so the live path runs
  through Kafka, not the database, which holds the durable record and the
  features and audit rows written back (in-memory fallback when
  unavailable).
  ...
  fail-closes to ABSTAIN at the assessment stages.  There, QC and anomaly
  scoring run before the FSM evaluates the resulting score, so the control
  lines to those two agents show the target arrangement, not the current
  call order.}
...
Two processing agents run in the deployed worker: QC annotates
ingested records as best-effort audit metadata, and the Anomaly
detector scores the retained window and drives the deterministic FSM.  Scenario, Verification and Report are
specified with the same typed handoff schemas, and nothing outside the
offline demo scripts invokes them (Figure~\ref{fig:pipeline}).

\paragraph{Data Plane.}
Three ingest connectors poll external NOAA and USGS endpoints at
configurable intervals (DART: 60\,s standard / 15\,s event mode;
CO-OPS: 30\,s; USGS seismic: 15\,s);
the full DART and CO-OPS station network is shown in
Appendix~\ref{sec:appendix-coops}. Standard-mode DART
...
validation are quarantined. The payload hash is carried in the input-reference field of the
handoff schemas where populated; end-to-end
...
agents (QC, Anomaly, Scenario, Verification) are specified to execute
sequentially; only QC and Anomaly execute in the deployed worker.
...
permissions are declared in a machine-readable policy matrix that
documents the intended capability envelope; nothing in the execution
path consults it, so it does not enforce anything at runtime
(Section~\ref{sec:safety}).

\paragraph{Output Plane.}
The Report Agent is specified to produce structured probabilistic
assessments in three tiers and is exercised only by the offline demo
scripts. The ABSTAIN output node produces an abstention document
whenever a verification result is missing, malformed, INCOMPLETE or
FAIL; the missing case is the one that fires. Reports and abstentions alike are presented at
the Human Review Gate, which is the precondition for any future
classification as \texttt{APPROVED\_INTERNAL}
(Section~\ref{sec:scenario}).

\paragraph{Cross-cutting concerns.}
Each pipeline run in the API and the deployed worker is wrapped in an
OpenTelemetry~\citep{opentelemetry2024} trace span, exported to Jaeger
when an OTLP endpoint is configured; per-agent spans are future
work. A metrics endpoint reports the current FSM state, domain
counters (ingest outcomes, anomaly scores, FSM transitions, abstains,
verification outcomes, guardrail scans), and a per-station
scoring-latency histogram; end-to-end latency histograms are future
work. Audited system events are appended to an audit log held in
memory as a 10{,}000-entry ring that evicts its oldest entries, and
persisted to TimescaleDB on a best-effort basis when a database is
configured (Section~\ref{sec:audit}).
...
backend-for-frontend. Every API route except \texttt{/health} and
\texttt{/metrics} requires an API key.

\paragraph{Mission Control dashboard.}
A browser-based dashboard (Figure~\ref{fig:dashboard-layout},
Appendix~\ref{sec:appendix-dashboard}) streams FSM state, anomaly
scores, and audit entries to the duty scientist over a WebSocket
connection.  It shows the current state and its transition history, a
Pacific map of station markers and the epicenter with the anomaly
score time series and thresholds ($T_1$, $T_2$, $T_3$) along its lower
edge, the component registry, the active event, and the audit log.
Displays are read-only; the sole write path is the Human Review Gate,
which issues APPROVE, REJECT and DEFER decisions through a separate
service-authenticated endpoint. Reviewer identity on that
...
\paragraph{Target operational flow.}
The deployed worker already runs ingest, QC, anomaly scoring, and FSM
transitions on live streams.  The flow below is the target.  Of its
stages, only anomaly scoring has been exercised on archived data
(Section~\ref{sec:evaluation}); the replay scripts compare each
station's full-record score against the same threshold values the FSM
uses, rather than running ingest, QC or the orchestrator, or climbing
one state per evaluation as the FSM does, and the Scenario, Verification and
Report stages remain future work that no path runs on live or archived
streams.
The seismic connector polls the USGS feed every 15\,s by default, so
detection is bounded by that interval plus the round trip, and the FSM
...
The QC Agent runs five QARTOD-aligned checks over each incoming
observation as it enters the detection pipeline.  All five can return
NOT\_EVALUATED rather than a verdict: gross range does so for every
DART record, and the other four do so on a station's first record,
before there is a previous observation or a full rolling window to
compare against. The resulting flags
...
\begin{description}[leftmargin=1.5em]
  ...
  \item[Gross range check.] Flags values outside configured QC
    ...
    pipeline.  The ingest layer rejects non-finite values (NaN and
    infinities), and NDBC 9999.000\,m
  ...
  \item[Rate-of-change check.] Flags records for which
...
and \texttt{MISSING}).
Records with $c < 0.5$ are marked unusable. As noted above, the mark is
advisory: anomaly scoring consumes every record regardless. The
deployed worker summarizes each batch of QC results into the audit
trail.
...
needs only 3~hours of prior data, which is why it remains operational
in NOAA's Tsunami Detection Algorithm~\citep{mofjeld1997tsunami}, but
it trades waveform characterization accuracy for that speed.  We
accept the 30-day calibration constraint in exchange for the
...
to isolate the tsunami frequency band.  Because the filter is causal,
the filtered value at time $t$ depends only on observations at or
before $t$, which is what real-time processing requires.
...
summarized in Algorithm~\ref{alg:residual}.  One step acts on the raw
record before detiding: a median despike that removes single-sample
telemetry glitches.  Three more act on the residual afterwards, a
pre-event linear detrend, a centered rolling-median baseline
correction, and a Hampel spike clean.  The algorithm applies its
Hampel and linear-detrend equivalents to the calibration window only
and has no rolling-median stage, so these figures are processed
slightly further than it specifies.  Each caption gives the
rolling-median window used for that event.
...
to~1 at $A_\text{max} = 2\theta$. Background noise below the station threshold therefore adds
nothing to the ensemble. In quiet periods (no M$\geq$6.5 event within 90\,min),
...
Tsunami onset shows up as a distributional shift in the broadband
residual before the narrowband oscillatory waveform develops in the
5--120\,min passband, so we apply the BOCPD
algorithm~\citep{adams2007bayesian} to that broadband residual.
...
300~samples).
Under constant hazard, the marginal posterior $P(r_t{=}0
...
so the changepoint itself cannot inflate the scale estimate.
...
yielding $s_\text{bcp} \in [0,1]$.  This component matters most for DART
...
the source axis will not confirm.  The gap affects no reported number,
since $s_\text{coh}$ enters only the Isolation Forest feature vector
and that model is never loaded.

\paragraph{Ensemble fusion.}
The spatial coherence score $s_\text{coh}$ is not weighted directly in
the sum; it is one input feature to the Isolation Forest, alongside
short-period energy and rate-of-change features, which produces the ML
anomaly score $s_\text{ml}$.
...
The expected Rayleigh arrival time is
...
A firing correlation produces CONCERN, never FAIL: timing
correlation is not proof of a false trigger, since real tsunamis also
follow large earthquakes.  A clean check passes, and a check whose
prerequisites are missing is recorded as not evaluated.
...
transition IDLE to MONITOR; an event outside those zones does not
leave IDLE regardless of magnitude. Most subsequent transitions are
driven by the ensemble anomaly score $s$ against three configurable
thresholds: $T_1 = 0.35$, $T_2 = 0.60$, $T_3 = 0.85$
(Table~\ref{tab:fsm}).  Four paths do not turn on the score: an
override ($M \geq 7.5$ with DART event-mode transmission) advances the
FSM one state per evaluation and takes priority over de-escalation, a
seismic-only chain ($M \geq 7.5$, depth $< 100$\,km) escalates before
any score exists, a 12-hour dwell timeout returns MONITOR to IDLE, and
an explicit \texttt{resolve\_event()} call returns ESCALATE to IDLE.
These thresholds are design constants, spaced to separate the
escalation stages, with $T_3$ set high to reduce false escalation.
They are not calibrated against historical events (see Limitation~6 in
Section~\ref{sec:discussion}).

\begin{table}[t]
\centering
\caption{FSM state transitions. The De-escalation column gives the
  automatic transition that reverses the row's own transition, where one
  exists, firing when the score drops below the corresponding lower
  threshold.  The MONITOR timeout back to IDLE is a row in its own right.}
\label{tab:fsm}
\begin{tabular}{llll}
\toprule
From & To & Trigger & De-escalation \\
\midrule
IDLE        & MONITOR     & Seismic $M \geq M_\text{min}$$^\P$ & --- \\
...
INVESTIGATE & ASSESS      & $s \geq T_2 = 0.60$           & $s < T_2$ $\to$ INVESTIGATE$^*$ \\
ASSESS      & ESCALATE    & $s \geq T_3 = 0.85$           & --- \\
...
INVESTIGATE is suppressed (the FSM advances to ASSESS instead).  The
same holds at ASSESS, where the override is evaluated before the
$s < T_2$ check and so escalates rather than de-escalates.\\
$\P$ The magnitude test is not sufficient on its own: the event region
must also be one of the configured tsunamigenic zones, or the FSM stays in
IDLE however large the earthquake.\\
$\S$ Explicit event resolution exists in the state machine but no
API route invokes it, and the
...
\draw[fwd]
  ...
  node[lbl, midway, above=9pt, align=center]
    {$M{\geq}M_\text{min}$,\\[-2pt]tsunami zone}
...
\caption{\textbf{FSM state diagram.}
  ...
  bypasses \textsc{investigate}/\textsc{assess} entirely, firing as a
  chained \textsc{idle}\,$\to$\,\textsc{monitor}\,$\to$\,\textsc{escalate}
  on the triggering event rather than from a standing \textsc{monitor};
...
Each transition is appended to the audit trail as an immutable record
of the states it moved between, the trigger reason, the anomaly score
and seismic magnitude that drove it, and the threshold values in
effect at the time. The FSM is not thread-safe internally; the FastAPI application
serializes its own in-process access via a single lock.  The pipeline
worker holds a separate instance in a separate process, and the
architecture assumes one FSM-writing worker.
...
\ELSIF{$q = \textsc{monitor}$}
  ...
  \ELSIF{$\Delta t \geq 12$\,h \AND $s < T_1$} \RETURN \textsc{idle}
  \ENDIF
...
The Scenario Agent is specified to run when the FSM reaches ASSESS or
ESCALATE, solving an NNLS
inversion~\citep{lawson1974solving, percival2011extraction} against
...
constraints). The active stage is specified to be recorded with the
assessment and carried into the final report.

\paragraph{Seismic-only estimate.}
The Scenario Agent defines a seismic-only estimate from magnitude
scaling alone, for the case where the FSM enters MONITOR and no DART
data are yet available.  Surface rupture length follows the all-fault-type regression
...
estimate. No DART constraint. High uncertainty.''} No runtime path
invokes it: the offline demo script is its only caller, and the
deployed worker emits a seismic-only ABSTAIN artifact instead.  Were
it produced, it would remain PROVISIONAL and non-distributable
throughout: a review decision is
recorded against it but does not change its status, and the system has
no capability to send any output beyond the operator interface.

\subsection{Verification and ABSTAIN Enforcement}
\label{sec:verification}

The Verification Agent defines nine checks over a scenario assessment.
They are policy rather than observed behavior: no runtime component
assembles the input they read, so none of the nine has run on live
data. Together they encode the requirement that
...
\caption{Verification checks. Any FAIL verdict triggers
  \texttt{abstain\_required=true}. No runtime component assembles a
  \texttt{VerificationInput}, so none of the nine runs on live data.
  Three of the four rows marked $\dagger$ read a
  \texttt{VerificationInput} field that nothing sets, so they report
  NOT\_EVALUATED for a missing prerequisite; the tidal state row is instead
  unreachable because the inversion whose coastal proxies it inspects does
  not run.
...
Tidal state$\dagger$ & Needed + uncorrected          & Uncorrected    \\
                     &                               & coastal proxies\\
Meteotsunami$\dagger$ & Meteo.\ score $> 0.6$        & $> 0.3$        \\
...
Two independent defenses hold this invariant, one of which is live
today. The verification result schema carries a model validator
that:
\begin{enumerate}
...
\end{enumerate}
The pipeline router is fail-closed independently of the schema:
missing, malformed, INCOMPLETE, and FAIL verification results all
route to the ABSTAIN path.  The missing case is the one that fires today, since the
live worker reaches ASSESS and ESCALATE with no verification result at
all and abstains there. The ABSTAIN output is a structured document recording the reason
...
emitted unchanged either way) synthesized by a four-node LangGraph pipeline:
...
dropped and the deterministic template stands alone. The LLM cannot modify the deterministic report text, the numerical
results, the report tier, or the assessment status. Full prompt text is provided in
...
deterministic report intact.  The test suite exercises 74~guardrail test functions with
126~assertions covering prohibited-term detection, disclaimer
...
model-authored tool-call logs scanned as structures rather than as
serialized JSON,
...
What keeps this advisory is enforced by the database, not asked for in
the prompt.  Findings are written to a table that migration 009 grants to a
...
neither written nor read by the code that escalates.  Migration 009 makes
exactly one grant on that table, and the integration suite asserts the
insert denial against a provisioned database.  The investigator runs on request, and only once the worker has
persisted a checkpoint. It binds its findings to that row and reads
the event's audit trail for evidence, so it sits off the detection
path and cannot add latency to ingest, scoring, or a transition. Every
finding passes the same guardrail scanner as any other emitted
narrative, and a violating finding is dropped whole rather than partly
rewritten.  Re-running an issue over the same
...
This is advice about evidence, not a second opinion on the hazard.  Nothing downstream consumes a finding: it does
...
detection. Reviewer identity on this endpoint is caller-asserted, and no code
path acts on the decision: none of the three closes the event,
authorizes distribution, or changes FSM state, and the assessment
stays \texttt{PROVISIONAL}.
...
What is enforced sits in three places, none of them the matrix. The
connectors issue GET requests only, and the deployment holds no
credential that writes to any NOAA system, so the software cannot
write to an operational feed, and it has no capability to disseminate
outside the deployment or to actuate anything physical. No role is
granted UPDATE or DELETE on \texttt{audit\_events}, the four baseline
roles have both explicitly revoked, and a database trigger blocks them
regardless of grants. In code, the guardrail scanner, the schema
validators, and the FSM transition rules each refuse the inputs they
are written to refuse.
This layering follows the Simplex Architecture principle of checked
redundancy~\citep{sha2001using}, though nothing here has undergone a
formal safety integrity level assessment.

\begin{table}[t]
\centering
\caption{Permission codes. Each agent's allowed codes are declared in
  \texttt{policy/permissions.yaml}. The declaration is queryable but is
  not consulted during execution; see the text.}
...
\label{sec:audit}
Every audited step is appended to an audit log, across roughly two
dozen event types covering FSM transitions, quality-control and
anomaly results, guardrail scans, permission and policy checks,
provenance capture, escalation packets, review decisions, abstentions,
and pipeline outcomes.  The in-memory trail is a bounded
10{,}000-entry ring, so a long-running process without a database
eventually evicts its oldest entries; the durable copy in TimescaleDB
is the one that keeps everything.  Each entry is an \texttt{AuditEntry} carrying a UUID,
UTC timestamp, event ID, pipeline trace ID, event type, producer, and
a full data snapshot. A transition writes its entry before the
in-memory state changes and aborts if that write raises, so the
in-memory trail cannot skip a transition; the database write is a
separate best-effort transaction whose failure is logged while
processing continues.
No application role is granted UPDATE or DELETE on
\texttt{audit\_events}, the four baseline roles have both explicitly
revoked, and a row-level trigger raises on either regardless of
grants, which binds the owning role too. Each connector records a SHA-256 hash of the raw payload at the ingest
boundary and carries it in the handoff schemas, separately from the
record it covers, so re-hashing an archived observation and comparing
it against the reference detects any alteration between ingest and
wherever that reference reached.  Nothing performs that comparison
automatically, and the reference does not yet travel the whole way, so
the check is one a reviewer runs rather than a property the pipeline
enforces. FSM transition and decision audit metadata is queryable by event ID or trace
ID via the API; complete raw-to-output \texttt{input\_refs} provenance is not
yet exposed end to end.
Appendix~\ref{sec:appendix-trace} annotates a synthetic pipeline run
for the M8.5 Aleutian and M7.0 scenarios.  The QC, anomaly and FSM
values come from execution; the scenario and verification entries are
illustrative schema examples.

\paragraph{Single-event FSM constraint.}
The FSM tracks one event at a time. A new seismic trigger arriving
while the FSM is non-IDLE is dropped with a warning log, so a second
qualifying earthquake during an open event receives no assessment at
all. Multi-event tracking is future work.
...
Those scripts call the scientific components directly on archived
files.  They do not run the deployed worker path, so nothing reported
here measures live behavior.  All five events were available while the
detector and its constants were being developed, so these are
retrospective replays rather than a held-out test.  Throughout this
section, detection means an ensemble score crossing a threshold on an
archived record: not a warning, not an alert, and not a forecast.
...
subduction-zone thrust events along the Pacific Rim and one outer-rise normal-faulting event (Samoa 2009), so the set covers
two source mechanisms rather than one.
...
Event mode alone does not guarantee a crossing: 46403 for T\={o}hoku
stays below $T_1$ throughout, and 46412 for Chile ends the full window
below it, crossing only briefly late in the record. Distance and
signal strength still govern the outcome.
...
the earthquake origin marked.  At the near-field stations the tsunami appears as a sustained
oscillation beginning minutes to hours after the earthquake, with
smaller amplitude at greater distance; the most distant records
contain seismic pressure perturbations rather than a tsunami arrival
inside this window.
...
At $M_w$\,9.1 with depth 29\,km, the seismic-only escalation path fires on the seismic parameters
alone: the FSM rules take it IDLE~$\to$~MONITOR~$\to$~ESCALATE on the
USGS trigger, which would place the event in human review before any
DART data arrived.  In parallel, the Anomaly Agent begins processing
incoming DART observations.  Station 21418 (560\,km) crosses $T_1$ and $T_3$ at +3.6\,min.  That
crossing is a pressure excursion consistent with coseismic seafloor
deformation and long-period seismic energy on the recorder, not an
independent tsunami confirmation.  In the full flow the QC Agent would
annotate each record with QARTOD flags and confidence as audit
metadata; the Tohoku validator does not run it, and the archived
records carry DART measurement-type codes marking all eight stations
in event mode.  The Scenario Agent would then produce a seismic-only
Wells \& Coppersmith estimate, followed by a DART-constrained
NNLS inversion once sufficient waveform data accumulates.  The
Verification Agent would then evaluate the nine-check matrix, though at
the seismic-only constraint stage most of those checks either do not
apply or have no input, and the Report Agent would render the
deterministic template report with an optional LLM narrative attached
(bounded autonomy: synthesize and explain only, no state
modification).  The Human Review Gate presents the escalation packet
for APPROVE, REJECT, or DEFER.
The FSM rules put this event through IDLE~$\to$~MONITOR on the
$M$\,9.1 trigger and MONITOR~$\to$~ESCALATE on the seismic-only path
(depth 29\,km $< 100$\,km).  The Tohoku validator does not
instantiate the orchestrator, so no audit trail was recorded for this
event; the verification and report entries the full flow would add
come after stages nothing outside the offline demo scripts assembles
input for.
Any assessment reaching the gate stays PROVISIONAL. A human operator
records APPROVE, REJECT, or DEFER against it; under caller-asserted
identity none of these changes the status, which would otherwise
become APPROVED\_INTERNAL.  Appendix~\ref{sec:appendix-trace} gives a
...
\caption{Detided residuals after removing the 8-constituent harmonic
  tidal prediction fitted on 30-day calibration data.  At the
  near-field stations the isolated tsunami signal attenuates with
  distance.  In the most distant panels the excursions are seismic
  pressure perturbations, because the tsunami cannot reach those
  stations inside the plotted record; 46408 sits at that boundary,
  with its predicted arrival near the end of its record.
  Processing: a despike on the raw record before detiding, then a
...
$M_w$\,8.1--9.1.  Samoa is the one case outside subduction-zone thrust faulting, and the
detector reached the same operating points there.  One outer-rise
normal-faulting event is not enough to establish that the detector
generalizes across mechanisms.
No station sampling at the 15-minute standard interval crossed $T_1$ in
any of the five events; every crossing came from a station in event
mode, whose passband is not clamped by the Nyquist limit.  Sampling
rate is not the only difference between those two groups, though.  Of
the thirteen standard-mode station records, twelve sit farther from
their epicenter than a tsunami travels in the six-hour evaluation
window at the shallow-water speed of 198\,m\,s$^{-1}$, so for those
records neither the tsunami nor the coseismic signal could have
arrived in time to be scored.  The low standard-mode scores therefore
cannot be read as a measurement of what coarse sampling costs.

\FloatBarrier
\subsection{Detiding Validation}
\label{sec:eval-detiding}

Anomaly detection quality depends on tidal removal accuracy.
...
The holdout RMS measures out-of-sample tidal prediction, which is why
the two elevated values above matter: at those stations the harmonic
model does not carry beyond its fitting window.
...
The design expectation is that near-field stations, which receive
higher-amplitude arrivals, activate the threshold and wavelet
detectors, while far-field stations, which receive attenuated signals,
depend more on BOCPD detecting a distributional shift in the broadband
residual.  The ablation in \S\ref{sec:eval-ablation} tests that
expectation against the scores.
...
This also settles the far-field expectation raised in
Section~\ref{sec:eval-components}, and it settles it both ways.  At
46403 BOCPD is the only statistical component that responds at all
(0.263, with the wavelet at zero), which is the expected pattern.  At
46411 the wavelet dominates instead (0.753 against BOCPD's 0.369).
Two far-field stations do not decide the question either way.
...
To measure the quiet-period false trigger rate, we process 30~days of
...
than against periodic signals).  Under Eq.~\ref{eq:ensemble_noml} the observed 0.80 is reproduced by a
saturated threshold score with a wavelet score near 0.52 and BOCPD
silent, so the threshold detector supplies 0.59 of the 0.80 and the
wavelet detector responds to the same transient rather than firing
independently.  The committed artifact records only the ensemble, so
those component values are not among the published results.  An event-mode (1-minute sampling)
...
For each configuration we record the highest threshold band the
ensemble score reaches, labeled with the FSM state that band selects.
The orchestrator is not driven here: the script compares scores
against the same threshold constants the FSM uses. Figure~\ref{fig:synthetic-heatmap} shows
...
\caption{Synthetic detection sensitivity heatmap (noise = 0.001\,m,
  sampling = 60\,s). Each cell shows the highest threshold band the
  ensemble score reaches for a given (amplitude, period)
  configuration, labeled with the FSM state that band selects.
  The ASSESS band ($T_2 \le s < T_3$) rarely appears because signals
  strong enough to cross $T_2$ typically also cross $T_3$ within the
  analysis window, so most cells fall either below $T_1$ or above
  $T_3$.}
...
The verification layer (\S\ref{sec:verification}) degrades
conservatively when observational data are insufficient.  What follows
is the aggregation policy and the unit tests over it.  Verification
does not run in the retrospective replays or the synthetic scenarios,
so none of it is measured on event data.
...
Table~\ref{tab:latency} reports component latencies.  QC, detiding,
anomaly scoring and the FSM transition are measured; the ingest rows,
scenario inversion and both report paths are design targets, because
no run of them is profiled; the review gate is an operator decision
and has no bound at all.  Measurements come from
...
$\sim$47\,ms at p95, of which detection over an already detided
residual is 44\,ms.  No component below that row is profiled
separately in the artifact, so the split across threshold, wavelet and
BOCPD is not measured here. QC checks process 10 records
in ${\sim}0.16$\,ms at the same percentile, and FSM transitions are
sub-millisecond. Against a 1-minute event-mode sampling interval, single-station
compute is not what bounds the deployed worker; the ingest poll
interval and the network round trip are, and those rows are design
...
To exercise the detection path end to end under controlled conditions
...
They test whether the software carries a signal through detiding,
filtering, and ensemble scoring, and whether the resulting scores fall
on the intended side of the thresholds.  The orchestrator itself is
not driven in this path: the script compares scores against the same
threshold constants the FSM uses.  The retrospective evaluations on real Tohoku and Chile
DART records (\S\ref{sec:eval-tohoku}; additional events in Appendix~\ref{sec:appendix-case-studies})
are the only tests here against observed data.  Those events were also
available during development, so they check the detector against real
signals without being an independent benchmark.
...
different from the Tohoku case study.  Because the waveforms come from
the same simulator, this varies the geometry of the test rather than
showing that detection carries to new source regions.

\paragraph{Scenario~2: Moderate earthquake (M7.0 Aleutian).}
Four DART stations: all ensemble scores remain below $T_1 = 0.35$, so this scenario
produces no escalation at a characteristic deep-ocean amplitude of
${\sim}0.009$\,m.
...
The four scenarios exercise the detection path on inputs the simulator
built from the same assumptions the detector uses, so they show the
software behaving as designed on those inputs.  They do not measure
how the detector responds to signals the simulator does not
reproduce.
...
\caption{Four-scenario physics simulation summary.
  ...
  (c)~Meteotsunami: no escalation; the 0.10\,m amplitude is below
...
CO-OPS tide gauges).  DART station amplitudes (0.025--0.032\,m) follow from two terms the
simulator applies: geometric spreading over distances of
2{,}448--3{,}764\,km, and azimuthal directivity, where the Aleutian
fault strike (250\textdegree) puts maximum radiation toward
340\textdegree, while the six DART stations lie west-southwest
(243--261\textdegree) or east (85--94\textdegree) of the epicenter and
receive 41--50\% of the peak amplitude.  The three CO-OPS stations lie
south (150--189\textdegree), against the opposite lobe of the same
$\cos^2$ pattern and so near a radiation maximum too, so their
${\sim}9\times$ larger amplitudes (0.23--0.28\,m) come from Green's Law
shoaling (${\sim}3.8$--$4.5\times$ at nearshore depths of 10--20\,m)
multiplied by a directivity factor $1.7$--$2.4\times$ higher than the
DART stations receive.
...
M7.0 event, the scores remain near zero throughout: the injected
amplitude stays below the station threshold, so the threshold
...
\caption{Sliding-window detection score evolution.
  ...
  Right column: M7.0 moderate earthquake (three stations); all scores
  remain below $T_1 = 0.35$.
...
flexibility but at the cost of determinism and auditability. Hours after an event, a reviewer has to be able to reproduce exactly
the reasoning that produced each assessment. The
...
3~hours for Mofjeld) for physically interpretable detiding and
multi-component ensemble scoring. The computational overhead of the
ensemble (${\sim}47$\,ms p95) is negligible relative to the data
polling interval (15--60\,s). It also defines a nine-check
verification layer, a review gate and a structured abstention, which
the compared methods do not. Nothing outside the offline demo scripts
assembles the verification input, so that column of
Table~\ref{tab:method-comparison} describes shipped policy rather
...
The four-node synthesis graph attaches LLM-generated operator
narratives to the deterministic pipeline without changing what that
pipeline decides. Three
...
detection pipeline (detide, bandpass filter, wavelet, BOCPD, ensemble
fusion) completes in ${\sim}47$\,ms (p95) for single-station data on
the profiling machine (Table~\ref{tab:latency}, AnomalyAgent
end-to-end row).  The Isolation Forest is not loaded, so it
contributes nothing to that figure. Scenario inversion dominates the end-to-end
budget because the NNLS solver (Eq.~\ref{eq:nnls}) iterates over the propagation database;
the 3-minute figure is a design target rather than a measurement,
assuming a pre-loaded database and the at most 15 candidate unit
sources the selector returns by default. When the LLM synthesis layer is enabled a single API
call adds seconds rather than milliseconds, and the template-only path
is what survives an API failure; neither cost is profiled here. The
end-to-end target of ${\leq}5$ minutes from data availability to
Tier~1 report is within the same order of magnitude as
...
Detide + bandpass & 0.89 & 1.10 & Harmonic fit + Butterworth (measured) \\
Anomaly detection (full) & 43.5 & 44.4 & Threshold, wavelet, BOCPD, fusion (measured) \\
...
In all five events the nearest event-mode station crossed $T_1$,
though the tier it reached ranged from INVESTIGATE to ESCALATE.
The earliest detections at the
...
804\,km) reaches only INVESTIGATE at 0.588 while 51426 at 932\,km
...
distances from 292 to 15{,}840\,km. Because all five events were
...
compared with a 235.9~min reference at 2{,}803~km. Event-mode status is
...
Figure~\ref{fig:component-strip} shows component-level score
...
Figure~\ref{fig:noise-triptych} shows how detection degrades across a
$10\times$ range of ambient noise levels
...
\caption{Detection sensitivity heatmaps at three ambient noise levels
  ...
  amplitude range at 90--120\,min periods).}
\label{fig:noise-triptych}
\end{figure}

\paragraph{Limitations.}
The current implementation has several known constraints. First,
durable state depends on an optional component: without a configured
TimescaleDB the FSM state and audit log live only in process memory,
and a restart clears them. With a database configured, both persist
and are recovered on restart. Second, the system
...
point is that nothing outside the offline demo scripts assembles a
\texttt{VerificationInput} at all: the pipeline node that would run these checks reads a
...
set \texttt{abstain\_required} and route to ABSTAIN.  At this stage, though, an abstention rests on very little evidence.
...
destructive''~\citep[pp.~31--32]{gonzalez2005nthmp}.  Separately, the earliest sliding-window detections per event
...
for this submission, because rescaling alone makes things worse.  Over
the same 414 June 2011 quiet-period windows, a power-density ratio
takes the false-trigger counts from 1/1/0 at $T_1$/$T_2$/$T_3$ to
2/1/1, that is from $0.29$ to $0.57$ false $T_1$ triggers per
station-month, and introduces a $T_3$-level false alarm at
station~46408, which rises from $0.802$ to $0.997$.  That run is not
among the committed artifacts, so those revised rates are reported
here as a measurement we made rather than one a reader can
reproduce.  The thresholds were chosen against the current suppressed
...
currently performs ingest, QC, anomaly scoring and FSM transitions,
builds a per-checkpoint assessment and, on entering ESCALATE, the
reviewer packet of record where a database is configured, and
...
confirmation.  An event whose ocean signal never lifts the ensemble
score to $T_3$, and which lacks the DART event-mode activation the
override requires, can still reach ESCALATE through the seismic-only
path (Section~\ref{sec:fsm}), which carries no sea-level evidence at
all.  The routes into ESCALATE therefore do not rest on comparable
grounds.
...
Twentieth, two properties named as safety controls above are weaker in
the implementation than their names suggest.  Reviewer identity at the
Human Review Gate is caller-asserted rather than authenticated, so an
APPROVE record shows that some authorized caller decided, not which
person decided; for that reason no decision closes an event or lifts
an assessment out of \texttt{PROVISIONAL}
(Section~\ref{sec:report}).  The agent permission matrix is likewise a
declaration.  It is queryable and documents each agent's intended
capability envelope, but no pipeline node consults it during
execution; the constraints that actually hold are database role
grants, the structural separation of the LLM layer from the control
path, and the review gate itself (Section~\ref{sec:safety}).

\paragraph{Future work.}
Four items follow directly from the limitations above and take
priority.  The inversion needs a real unit-source propagation database
before its amplitudes are worth reporting.  The dimensional correction
to the wavelet score has to ship together with a recalibration of
$T_1$, $T_2$ and $T_3$ against a larger event archive.  The review
gate needs authenticated reviewer identity and a separate
event-disposition record.  The deployed worker needs to be exercised
end to end through scenario inversion and verification on a live
stream.  Beyond those, planned extensions include a multi-event FSM
for concurrent tracking of independent seismic triggers, integration
with the NOAA CO-OPS one-minute tsunami product, and Kubernetes
...
The intended beneficiary is the duty scientist, who currently
assembles a picture of a developing event by hand from several
separate feeds and tools.  Whether this framework shortens or improves
that work is untested: no operator study has been run, and the
evaluation here measures detection behavior on archived records, not
decision quality.  The system itself issues nothing and reaches no
member of the public, so any benefit and any harm both pass through a
human first.

The system explicitly does \emph{not} replace or
interfere with NOAA's official warning systems. All emitted assessment
reports carry non-authoritative disclaimers. The system has no public
dissemination path at all, and the Human Review Gate stands between
any assessment and an approved internal status. The permission matrix documents each component's capability envelope
but does not enforce it; the constraints that hold are database role
grants, the separation of the LLM layer from the control path, and the
review gate. The append-only audit trail
supports independent review of FSM transitions and human review
decisions, and of verification outcomes wherever a caller supplies
them; complete raw-to-output
\texttt{input\_refs} provenance is not yet exposed end to end.

Potential risks include over-reliance on the system's outputs if duty
scientists do not maintain independent judgment, and misinterpretation
of the \texttt{APPROVED\_INTERNAL} status as an official product.  The
disclaimer carried on every emitted report and the framing of the
system as a situational awareness tool are partial answers to both,
but neither is evidence that an operator under time pressure reads a
disclaimer, and we have not tested that.  The code and design
documents are public so that the claims made here can be checked
independently.
...
ocean hazard assessment and released its implementation.  A
deterministic five-state machine orchestrates the pipeline, escalating
and de-escalating on threshold rules, a magnitude-and-DART override, a
seismic-only path and a dwell timeout; an ensemble anomaly
detector combines three signal analysis components with an optional ML
component; NNLS scenario inversion produces bootstrap uncertainty
estimates; a fail-closed verification layer emits an explicit ABSTAIN;
an append-only audit trail carries SHA-256 hashes from the ingest
boundary; a bounded LLM advisory layer writes operator narratives that
are stored apart from the deterministic report of record and dropped
on failure or guardrail violation; and a Human Review Gate stands
between any assessment and an approved status.  Retrospective replay
on five Pacific tsunamigenic earthquakes ($M_w$\,8.1--9.1:
T\={o}hoku 2011, Chile 2010, Illapel 2015, Iquique 2014, and Samoa
2009) shows the ensemble detector crossing the INVESTIGATE threshold at the
nearest event-mode DART station in each event, and reaching ASSESS or
ESCALATE at that station in four of the five, across subduction-zone
thrust and outer-rise normal-faulting mechanisms.

Several things this framework contains were not shown here.  The
earliest detections reflect coseismic pressure perturbations rather
than confirmed tsunami arrivals, and the system does not tell the two
apart.  All five events were available during development, so
consistency across them is not a held-out result.  The scenario
inversion ran only against a development store of synthetic waveforms,
the verification layer was exercised in tests rather than on a live
stream, provenance is not yet carried end to end, and reviewer
identity at the review gate is caller-asserted.  Closing those gaps,
and calibrating $T_1$, $T_2$ and $T_3$ against an event catalog larger
than five, is what would have to happen before the framework could be
...
\caption{Component diagram showing the internal structure of the
  ...
  in-memory stores (the ingest workers persist raw observations and the
  pipeline worker persists features, audit and lineage records to
  TimescaleDB when configured). The Processing Plane holds the QC and
  Anomaly agents, which the pipeline worker runs, alongside the
  Scenario and Verification agents, drawn in their specified positions;
  nothing outside the offline demo scripts assembles their inputs
  today. The deterministic FSM
  sits beside them as the sole state authority. The Output Plane includes the Report Agent,
...
\caption{Specified pipeline sequence for a tsunami event assessment.
  The deployed worker executes ingestion, quality control, anomaly
  scoring and the FSM transition; the inversion, verification and
  report stages shown after them are specified but not assembled by
  any runtime path.
  ...
  scanning), and human review. Every step of that flow writes to the
  append-only audit trail (in-memory, with optional
...
\caption{FSM state machine diagram. Five states (IDLE, MONITOR,
  ...
  de-escalation paths, a 12-hour timeout returning MONITOR to IDLE, a
  seismic-only shortcut from MONITOR to ESCALATE ($M \geq 7.5$ and
  depth $< 100$\,km, no DART data required), and a separate seismic
  override ($M \geq 7.5$ with DART event-mode activation) that
  advances the score-driven states.}
\label{fig:uml-fsm}
\end{figure}
...
\caption{Scenario inversion activity diagram. DART-constrained and
  ...
  moment magnitude calculation~\citep{hanks1979moment}, bootstrap
...
\caption{After-action analysis graph with tool use. A 3-node LangGraph
  ...
  All tools are bounded (read-only queries), and the graph is compiled
  without a checkpointer, so each invocation starts clean.}
...
For reproducibility, we provide the system prompts for the two
report-time advisory graphs.  The active-event investigator's prompts
live in the same module and are omitted here for length.  All prompts are versioned under source control
...
These prompts drive the optional narrative layer.  It runs after the
deterministic outputs are fixed, never in the deployed worker, and it
cannot change a score, a state, or a verification outcome.
...
The narrative node's user message carries an
\texttt{<historical\_context>} XML block holding JSON from the
historical analogue lookup, and when
\texttt{system\_confidence}~$< 0.55$ an elevated uncertainty caveat is
appended to it.  The system prompt below is reproduced as it appears
in source.
...
INVESTIGATE & MONITOR & $s < T_1$ (de-escalation; suppressed when $M \geq M_{\mathrm{esc}}$ with DART event-mode$\to$ASSESS) \\
\addlinespace
ASSESS & ESCALATE & $s \geq T_3$ \\
ASSESS & ESCALATE & $M \geq M_{\mathrm{esc}}$ with DART event-mode activation (seismic override) \\
ASSESS & INVESTIGATE & $s < T_2$ (de-escalation; suppressed when $M \geq M_{\mathrm{esc}}$ with DART event-mode$\to$ESCALATE) \\
\addlinespace
...
bypasses the $T_3$ anomaly threshold from ASSESS.  It lets a large
earthquake with DART event-mode activation climb to ESCALATE on
successive score evaluations without the ensemble crossing $T_3$.  It
advances one state per evaluation, so it does nothing while no station
produces a scored window.  ESCALATE $\to$ IDLE fires only on an
explicit \texttt{resolve\_event()} call, which no API route makes
today; automated de-escalation from ESCALATE is intentionally absent.
...
The Mission Control console renders pipeline state in a
browser-based React interface.  The layout
...
The console is built for a duty scientist to watch the score timeline
and the transition ladder as an event develops; it has not been
exercised against a live event.  When the FSM reaches
\textsc{escalate}, the Human Review Gate activates and presents the
escalation packet of record.  The console shows the checkpoint, the
transition, the best scored station, and the assessment's identifier
and content hash; the assessment body travels inside the packet but is
not rendered.
...
The scientific handoff models in \texttt{schemas/} are strict Pydantic
v2 envelopes.  The traces below summarize the values each stage
produced in one synthetic run of
\texttt{scripts/run\_synthetic\_pipeline.py} (M8.5 Aleutian and M7.0
moderate scenarios); they are not the serialized envelopes, and not
every runtime boundary enforces them.
The QC, AnomalyAssessment, and FSM values come from actual pipeline
execution.  The ScenarioAssessment and VerificationResult entries are
illustrative examples of the output schema: that script runs the QC
agent, the detector and the FSM, and does not run the scenario or
verification agents.

\input{appendix_f_generated}
...
wavelet detection.  Two effects are confounded at these stations.  The
clamped passband degrades detection, and all four also sit beyond the
distance a tsunami covers in the six-hour window, so a low score here
measures neither effect on its own.
...
32411 & 4{,}920   & NNW, off Galapagos        & event (1\,min) \\
54401 & 8{,}740   & WSW, SW Pacific            & standard (15\,min) \\
46412 & 9{,}080   & NW, off San Diego CA      & event (1\,min) \\
46411 & 10{,}040  & NW, off N California    & standard (15\,min) \\
51407 & 10{,}740  & WNW, Hawaii              & standard (15\,min) \\
46402 & 13{,}110  & NW, S of Dutch Harbor AK & --- (no data) \\
...
54401              & 8{,}740  & std   & 0.130 & 0.221 & 0.000 & 0.000 & ---        & ---        & ---        & \checkmark \\
...
Standard-mode stations produce near-zero scores.  The
\texttt{filter\_degraded} flag records the clamped passband
(Table~\ref{tab:chile-results}), but distance confounds it, so the
flag alone does not account for the zeros.
...
The detector sets \texttt{filter\_degraded} on the four standard-mode
records, whose 15-min sampling clamps the bandpass to the Nyquist
frequency; their scores appear in Table~\ref{tab:chile-results} and
are omitted from the event-mode figure.  The ensemble weights renormalize
from $(0.50, 0.35, 0.15)$ to $(0.588, 0.412)$ without the ML
component, which is not deployed.
The FSM rules put this event through IDLE~$\to$~MONITOR on the
$M$\,8.8 trigger and MONITOR~$\to$~ESCALATE on the seismic-only path
(35\,km).  The validator scores the archived DART records against
the same threshold constants the FSM uses; it does not instantiate the
orchestrator, so no audit trail was recorded for this event.
...
32401 & 1{,}250 & W of Iquique, Chile    & event (1\,min) \\
32412 & 2{,}110 & NW, off Peru           & event (1\,min) \\
51426 & 9{,}280 & WSW, near Tonga          & standard (15\,min) \\
46411 & 9{,}740 & NW, off N California    & standard (15\,min) \\
46407 & 10{,}110 & NW, off Oregon          & standard (15\,min) \\
46403 & 12{,}450 & NW, off Alaska        & standard (15\,min) \\
\bottomrule
\end{tabular}
\end{table}
...
0.99, above the ESCALATE threshold ($T_3 = 0.85$);
station 32401 at 1{,}250\,km scored 0.753 (ASSESS level, $T_2 = 0.60$).
...
events.  The four far-field stations remained in 15-min standard mode
within the 6-hour event window and lie beyond the tsunami's reach in
that window; their scores are reported but not used for detection
assessment.
...
$T_3$ at +5.0\,min; station 32401 (1{,}250\,km) reaches
$T_2$ but not $T_3$ (ensemble 0.753).
The FSM rules put this event through IDLE~$\to$~MONITOR on the
$M$\,8.3 trigger and MONITOR~$\to$~ESCALATE on the seismic-only path
(22\,km).  The validator scores the archived DART records against
the same threshold constants the FSM uses; it does not instantiate the
orchestrator, so no audit trail was recorded for this event.
...
32401 & 1{,}250   & event & 0.753 & 0.914 & 0.524 & 0.364 & \checkmark & \checkmark & ---        & --- \\
32412 & 2{,}110   & event & 0.493 & 0.539 & 0.120 & 0.427 & \checkmark & ---        & ---        & --- \\
51426 & 9{,}280   & std   & 0.000 & 0.000 & 0.000 & 0.000 & ---        & ---        & ---        & \checkmark \\
...
\caption{DART stations used in the Iquique 2014 retrospective validation
  ...
  lands at 45\,s on the two stations whose event window holds the most
  15-second-tagged rows, 96 and 80 against 20 and 16 on the other two. The
...
\textbf{Station} & \textbf{Dist.\,(km)} & \textbf{Location} & \textbf{Mode} \\
\midrule
32401 &    292  & WSW, off Arica, Chile   & event (1\,min) \\
32402 &    858  & S, off Caldera, Chile   & event (1\,min) \\
32412 & 1{,}652 & W, off Peru            & event (45\,s) \\
32413 & 2{,}803 & WNW, off Peru (far)      & event (45\,s) \\
51426 & 9{,}900 & WSW, near Tonga           & standard (15\,min) \\
46403 & 11{,}476 & NW, off Alaska        & standard (15\,min) \\
\bottomrule
\end{tabular}
\end{table}
...
station 32412 at +141.2\,min.  The 2.3 hours between the first $T_1$
crossing and the first $T_3$ crossing is why the seismic-only path
exists: it moves the event to ESCALATE on the origin parameters while
DART scores are still accumulating.
The FSM rules put this event through IDLE~$\to$~MONITOR on the
$M$\,8.2 trigger and MONITOR~$\to$~ESCALATE on the seismic-only path
(25\,km).  The validator scores the archived DART records against
the same threshold constants the FSM uses; it does not instantiate the
orchestrator, so no audit trail was recorded for this event.
...
\textbf{Station} & \textbf{Dist (km)} & \textbf{Mode}
  ...
  & \textbf{Deg} \\
\midrule
32401 &    292    & event & 0.698 & 1.000 & 0.000 & 0.266 & \checkmark & \checkmark & ---        & --- \\
...
32413 & 2{,}803   & event & 1.000 & 1.000 & 1.000 & 0.932 & \checkmark & \checkmark & \checkmark & --- \\
...
\textbf{Station} & \textbf{Dist.\,(km)} & \textbf{Location} & \textbf{Mode} \\
\midrule
51425 &    804  & NW, near epicenter     & event (1\,min) \\
51426 &    932  & SSE, near Tonga         & event (1\,min) \\
54401 & 1{,}960 & S, south of Samoa       & event (1\,min) \\
51407 & 4{,}250 & NNE, off Hawaii          & standard (15\,min) \\
46411 & 7{,}680 & NE, off N California    & standard (15\,min) \\
46403 & 7{,}720 & N, off Alaska          & standard (15\,min) \\
\bottomrule
\end{tabular}
\end{table}
...
station 51426 (932\,km) achieves ensemble score 0.99 (ESCALATE),
...
The three far-field standard-mode stations are filter degraded.  Only
51407 is close enough for the question to arise, and even there the
predicted arrival at 357.5\,min falls a few minutes after its last
sample, so its zero score does not separate the two causes either.
...
(wavelet and BOCPD both zero).  With the statistical term at zero the
renormalized weights leave the threshold component carrying the score
on its own, which is enough to clear $T_1$ but not $T_2$.
The FSM rules put this event through IDLE~$\to$~MONITOR on the
$M$\,8.1 trigger and MONITOR~$\to$~ESCALATE on the seismic-only path
(18\,km).  The validator scores the archived DART records against
the same threshold constants the FSM uses; it does not instantiate the
orchestrator, so no audit trail was recorded for this event.
...
51426 &    932    & event & 0.986 & 1.000 & 0.966 & 0.000 & \checkmark & \checkmark & \checkmark & --- \\
...
\caption{Detection pipeline signals and scores for Samoa 2009
  ...
  Insets below each score panel show the signal at mm scale around
  score onset, the first nonzero ensemble score (blue dashed line).
  Station 51426 (932\,km) shows the strongest detection
