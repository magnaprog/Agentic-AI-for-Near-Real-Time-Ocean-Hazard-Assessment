...
inversion~\citep{lawson1974solving, titov2005realtime}, and a
...
An optional LLM advisory layer synthesizes operator-readable
narratives under bounded autonomy: it cannot modify detection
state or bypass verification.
...
The complete implementation is publicly available as open
source.\footnotemark[1]
\end{abstract}
...
(items 1--2) and four domain applications of established patterns
(items 3--6), showing how proven safety-engineering and AI techniques
...
\begin{enumerate}
  ...
  \item A public open-source implementation with extensive automated tests
...
6\% of 324~papers address warning dissemination and communication (a
gap our system partially addresses) while calling for concrete guardrails beyond
guiding principles. Our bounded autonomy model declares each agent's
...
earthquakes ($M_w > 8.5$~\citep{hanks1979moment}), significant energy extends to 100--300\,s,
...
(default 6.0) triggers the transition IDLE $\to$ MONITOR. All
...
\midrule
IDLE        & MONITOR     & Seismic $M \geq M_\text{min}$ & N/A \\
...
\ENSURE Next state $q'$
\IF{$q = \textsc{idle}$ \AND $e$ with $M \geq M_\text{min}$}
...
methodology~\citep{gica2008development} (the current implementation
uses the NOAA NCTR Forecast Propagation Database interface; see
Limitations for access constraints).
...
\caption{Verification checks. Any FAIL verdict triggers
  \texttt{abstain\_required=true}. Rows marked $\dagger$ state policy
  that the shipped pipeline cannot currently reach, because no
  component supplies the input the condition tests: the
  posterior-stability comparison receives no previous assessment and
  no meteotsunami score is computed, so both report NOT\_EVALUATED,
  and the tidal FAIL condition needs a correction flag that nothing
  sets. See the fifth limitation in Section~\ref{sec:discussion}.}
...
Model fit            & RMSE $> 5$\,cm or bias        & RMSE $> 3$\,cm \\
                     & $> 1$\,cm AND $> 3\sigma$     &                \\
Tidal state          & Needed + uncorrected$\dagger$  & Uncorrected    \\
                     &                               & coastal proxies\\
Meteotsunami$\dagger$ & Non-tsun.\ energy $> 60\%$   & $> 30\%$       \\
Rayleigh wave        & ---                           & Timing match   \\
...
terms.  Six are official product terminology defined in NWS Instruction
10-701, the Tsunami Warning Center operations instruction issued under
NWS Policy Directive 10-7~\citep{nwsi10701,nwspd107}: ``Warning,''
``Advisory,'' ``Watch'' and ``Information Statement'' (Sections 2.1.1
to 2.1.4), ``Cancellation'' (Section 2.1.5), and ``Threat Message,''
the product the Pacific Tsunami Warning Center issues to its
international service area (Section 2.2).  The remaining two are not
product names but are scanned all the same.  ``Bulletin''
is the label that instruction gives the numbered messages of a product
sequence.  ``All Clear'' appears in neither document, and is scanned
...
the eight terms leaves no spelling undetected.  Deliberately out of scope
...
deterministic report intact.  The test suite exercises 56~guardrail test functions with
106~assertions covering prohibited-term detection, disclaimer
...
character injection.  An additional 20~synthetic scenarios
(\texttt{scripts/evaluate\_llm\_synthesis.py}) validate the full
LLM synthesis pipeline including guardrail scanning under varying
event magnitudes and verification outcomes.
...
(Section~\ref{sec:report}) are not evaluated here because the
propagation database requires restricted access and LLM narrative
...
APPROVED\_INTERNAL.  A complete trace example in Appendix~F
format is available in the supplementary material.
...
\caption{Detided residuals (tsunami waveform) after removing
  ...
  Hampel spike filter, linear detrend, and Butterworth bandpass
...
\caption{Detection pipeline signals and scores for near-field
  ...
  At 46411 the large spike at ${\sim}$33\,min is the seismic Rayleigh
  wave (${\sim}$3.6\,km/s); the inset reveals the earlier P-wave
  arrival (${\sim}$18\,min, ${\sim}$8\,km/s), a ${\sim}$1\,mm
  perturbation that exceeds the deep-ocean noise floor and triggers
  the score jump before the larger surface wave arrives.}
...
reaches $T_3$ at 7/8 stations; this confirms that the threshold
detector is the primary detection channel, with the statistical
detector providing complementary confirmation.
...
tsunami amplitudes of 0.23--0.28\,m at the shallow-water stations
(10--20\,m depth) versus 0.025--0.032\,m at the deep-ocean DART BPRs.
...
Together, these scenarios validate correct detection across distance
...
\caption{Raw synthetic records for the M8.5 Aleutian scenario
  ...
  gauges.  DART stations show ${\sim}$0.03\,m tsunami amplitudes on a
  tidal baseline with instrument drift; CO-OPS stations show
  ${\sim}$0.25\,m amplitudes after Green's Law shoaling amplification.
...
is designed so that duty scientists are informed of the system's
limitations rather than misled by silence.

\paragraph{LLM synthesis layer.}
The implemented 4-node synthesis graph shows that
LLM-generated operator narratives can be integrated into a
safety-critical pipeline without compromising determinism. Three
...
$\to$ scan $\to$ commentary dropped, template stands) activates
correctly when the
...
Third, the inversion depends on the NOAA NCTR Forecast Propagation
Database; access constraints limit geographic coverage.
...
check, so only one check is required to have run.  The four OPTIONAL
checks still execute, but none of them can fail on the inputs the
pipeline currently supplies, and they divide into two groups.  Posterior
stability and the meteotsunami discriminator report NOT\_EVALUATED with
a MISSING prerequisite, the first because no previous assessment is
passed for comparison and the second because no component computes a
meteotsunami score.  The tidal-state and Rayleigh checks do run, and
neither can reach FAIL: the tidal FAIL branch requires a
tidal-correction flag that nothing sets, and the Rayleigh check has no
FAIL branch at all, returning CONCERN at worst when the anomaly layer
marks a station suspect.
The consequence reaches past this stage.  A single OPTIONAL check
blocked on a missing prerequisite degrades the aggregate to
PASS\_WITH\_CONCERNS, and the meteotsunami check is OPTIONAL at all
three constraint stages with a prerequisite that is never satisfied.
PASS is therefore unreachable in the shipped configuration at every
stage, not only this one.  No computed verification outcome appears in
the released artifacts, because the offline evaluation stops at
detection and the live worker fail-closes before assembling a scenario;
the outcome shown in Appendix~\ref{sec:appendix-trace} is a hand-built
illustration of the envelope shape, labeled as such there.
...
has direct human safety implications, since delayed
or incomplete assessments can affect evacuation decisions.
...
operational deployment.  The open-source implementation and
...
(see Limitation~10 in Section~\ref{sec:discussion}).
...
\caption{CO-OPS 6-minute water-level records at four Pacific tide gauge
  ...
  Note: Hilo observations unavailable (gauge outage Feb.\ 24 -- Mar.\ 1, 2010).}
...
\caption{DART stations used in the Iquique 2014 retrospective validation
  ...
  event-mode station is the median spacing of its retained record
  rather than a fixed instrument setting. These records mix 15\,s and
  60\,s sampling, the 15\,s stretches falling in the hours after
  origin, with 45\,s spacings where samples are missing from a 15\,s
  stretch. The median therefore lands at 45\,s on the two stations
  that retained the most 15\,s samples and at 60\,s on the other two
  event-mode stations, which is a property of the record and not of
  the mode.}
...
threshold-heavy renormalization preserves detection even when
two of three detectors produce no signal.
Audit trail:
