...
\draw[arrow] ([xshift=5.5cm]proc.south) -- (tsdb.north)
...
\caption{System architecture and agentic pipeline. In the
  ...
  TimescaleDB.  The worker consumes the topic, so the live path runs
...
\caption{FSM state transitions. The De-escalation column gives the
  automatic downward transition available from the \emph{From} state, where
  one exists; it fires when the score drops below the corresponding lower
  threshold.}
\label{tab:fsm}
\begin{tabular}{llll}
\toprule
...
MONITOR     & INVESTIGATE & $s \geq T_1 = 0.35$ OR override$^*$ & --- \\
...
\caption{Verification checks. Any FAIL verdict triggers
  \texttt{abstain\_required=true}. No component assembles a
  \texttt{VerificationInput}, so none of the nine runs on live data.
  The four rows marked $\dagger$ are unreachable for a second reason:
  the field each of those conditions reads is never populated on that
  structure, so they report NOT\_EVALUATED for a missing prerequisite.
...
54401 & 8{,}740   & W, SW Pacific            & standard (15\,min) \\
...
51426 & 9{,}280 & W, near Tonga          & standard (15\,min) \\
...
32413 & 2{,}803 & NW, off Peru (far)      & event (45\,s) \\
